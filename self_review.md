...
## 1. 가장 잘했다고 생각하는 부분
강의 시간에 처음으로 소프트웨어 엔지니어링 원칙을 적용해 패키지 구조를 설계하고, 객체지향의 상속(Task -> RecurringTask)을 활용해 코드 중복을 줄인 점이 만족스럽습니다. 또한, 기능 구현에만 벅차했던 과거와 달리 PEP 8 스타일 가이드를 준수하고 pytest를 작성해 코드의 신뢰성을 높인 점이 만족스럽습니다. 그리고 무엇보다 이런 절차를 GitHub Actions를 이용한 CI(지속적 통합) 자동화를 구축한 것이 매우 만족스럽습니다.
...
또한 아래에서 확인할 수 있듯, 과제에서 요구한 모든 단계에서 생성형 인공지능을 썼습니다. 컴퓨터공학과에 재학중인 학생으로서 굉장히 부끄러워해야 마땅할 일이지만, 모든 절차에서 완벽에 가까운 결과물을 원해서 요구하는 절차를 끝낼 때마다 생성형 인공지능에 검수를 받은 것이 끝나고 보니 굉장히 아쉬운 선택이었던 것 같습니다. 그래도 제가 생성형 인공지능을 사용했다는 것을 무작정 숨기기보다는 이런식으로 기록해놓는 게 자기성찰을 위해서라도 의미있는 행동으로 여거 사소한 것도 전부 기록하였으니 이 부분은 참작해주시면 감사하겠습니다.

## 3. 다음에 개선하고 싶은 점
이번 프로젝트에서는 보너스 목표 중 하나인 테스트 커버리지(pytest-cov) 측정과 GitHub Actions를 이용한 CI 자동와까지 완료하여 코드의 안정성을 확인했습니다. 다음 기회가 있다면, 혹은 저 개인적으로 프로젝트를 할 때는 여기서 한 걸음 더 나아가 Sphinx를 활용해 공식 문서를 웹페이지로 배포하는 작업까지 도전해 보고 싶습니다. 그리고 위에서 언급한 대로 생성형 인공지능을 꽤나 무분별하게 사용했기 때문에 다음 프로젝트에서는 이런 습관을 줄여나가면서 소프트웨어 공학 원칙을 상기할 수 있으면 합니다.
...
* 질문 및 답변 활용 내용:
    ...
    6.  4단계 문서화 및 코드 스타일 점검을 위해 core.py의 get_summary 메서드에 요구사항인 사용 예시(>>>)를 포함한 docstring을 추가. 이후 pycodestyle 실행 과정에서 발생한 공백 및 줄 바꿈(W293, E302, W292), 줄 길이 초과(E501) 등의 PEP 8 스타일 경고를 해결하기 위한 코드 수정 가이드를 받아 오류 없이 적용.
        * W292 no newline at end of file, 파일 끝에 새로운 줄 생성
          W293 blank line contains whitespace, 띄어쓰기가 포함된 빈 줄
          E302 expected 2 blank lines, 2줄 띄어쓰기
          E501 line too long, 행 길이 초과
